...
async def session_(bot, message):
    try:
        ...
        if not isinstance(APP_ID, int) or len(APP_ID):
            return await bot.send_message(message.chat.id, "The APP_ID must be a 7 digit number. Please try again with /start.")
        start_hash = await bot.send_message(message.chat.id, "APP_ID received, now send API_HASH.")
        api_hash = await conv(
            bot,
            chat_id=message.chat.id,
